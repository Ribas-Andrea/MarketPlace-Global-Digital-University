...
describe('PUT /menus/:id', () => {
  ...
  it('Un administrateur peut modifier un menu', async () => {
    ...
    const response = await request(app).put(`/api/menus/${menuId}`).field('disponible', 'false').set('Authorization', 'Bearer token');
  ...
  it('Un membre de l’accueil ne peut pas modifier un menu', async () => {
    ...
    const response = await request(app).put(`/api/menus/${menuId}`).set('Authorization', 'Bearer token');
